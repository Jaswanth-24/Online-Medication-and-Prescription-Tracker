...
public class PharmacyService {
  ...
  public List<InventoryItemResponseDTO> listInventory(String email) {
    Pharmacy pharmacy = getPharmacyByEmail(email);
    return inventoryItemRepository.findByPharmacyOrderByIdDesc(pharmacy).stream()
  ...
  public InventoryItemResponseDTO updateInventoryItem(
      String email, Long id, InventoryItemRequestDTO request) {
    Pharmacy pharmacy = getPharmacyByEmail(email);
    InventoryItem item =
  ...
  public List<DeliveryResponseDTO> listDeliveries(String email) {
    Pharmacy pharmacy = getPharmacyByEmail(email);
    return deliveryRepository.findByPharmacyOrderByPrescribedAtDesc(pharmacy).stream()
  ...
  public DeliveryResponseDTO updateDeliveryStatus(
      String email, Long id, DeliveryStatusUpdateDTO request) {
    Pharmacy pharmacy = getPharmacyByEmail(email);
    Delivery delivery =
        deliveryRepository
            .findByIdAndPharmacy(id, pharmacy)
  ...
  private Pharmacy getPharmacyByEmail(String email) {
    ...
    if (user.getRole() != Role.PHARMACY) {
      throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Not a Pharmacy Admin");
    }
