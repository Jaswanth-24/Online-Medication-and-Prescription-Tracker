...
public class AdminDoctorController {
    ...
    @GetMapping
    public ResponseEntity<List<DoctorProfileDTO>> listDoctors(
            Authentication authentication
    ) {
        User admin = userRepository.findByEmailIgnoreCase(authentication.getName())
        ...
        return ResponseEntity.ok(
                adminDoctorService.listDoctors(admin)
        );
    }
    ...
    @GetMapping("/template")
    public ResponseEntity<Map<String, Object>> getDoctorTemplate(
            Authentication authentication
    ) {
        User admin = userRepository.findByEmailIgnoreCase(authentication.getName())
                .orElseThrow(() -> new IllegalStateException("Admin not found"));
        ...
        return ResponseEntity.ok(
                adminDoctorService.getDoctorTemplateForReact()
        );
    }
}
