...
public class PatientController {
  ...
  public ResponseEntity<JsonNode> updatePatientProfile(
      @PathVariable Long patientId, @RequestBody JsonNode payload, Authentication authentication) {
    User user =
        userRepository
            .findByEmailIgnoreCase(authentication.getName())
            .orElseThrow(
    ...
    if (!user.getId().equals(patientId)) {
      throw new ResponseStatusException(
          org.springframework.http.HttpStatus.FORBIDDEN, "Cannot update another patient");
    }
  ...
  public ResponseEntity<JsonNode> updateMedicineStatus(
      @Valid @RequestBody MedicineStatusUpdateDTO request, Authentication authentication) {
    User patient =
        userRepository
            .findByEmailIgnoreCase(authentication.getName())
            .orElseThrow(
  ...
  public ResponseEntity<JsonNode> updateDeliveryStatus(
    ...
    return ResponseEntity.ok(
        patientRecordService.updateDeliveryStatus(patient, medicineId, request.getStatus()));
  }

  @PostMapping("/doctor-rating")
  public ResponseEntity<JsonNode> rateDoctor(
      @Valid @RequestBody DoctorRatingRequestDTO request, Authentication authentication) {
    User patient =
        userRepository
            .findByEmailIgnoreCase(authentication.getName())
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        org.springframework.http.HttpStatus.NOT_FOUND, "User not found"));
    patientRecordService.rateDoctor(patient, request);
